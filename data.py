...
def plot_folded_data(oid, dr, data, period, offset=None, repeat=None, fmt='png'):
    if repeat is None:
        repeat = 2
...
def parse_figure_args_helper(args):
    ...
    return dict(fmt=fmt, other_oids=other_oids, min_mjd=min_mjd, max_mjd=max_mjd)
...
def response_figure_folded(dr, oid, period):
    ...
    img = plot_folded_data(oid, dr, data, period=period, repeat=repeat, fmt=fmt)
