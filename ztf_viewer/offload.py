...
through `LoopRegistry` rather than as a module-level singleton. That matters more than it looks:
`release()` never checks which loop it is on and `acquire()` only checks when it has to wait, so
one reached from the wrong loop miscounts in silence rather than raising. `to_thread` below is
the only thing that touches these, and it awaits on the loop the registry keyed.
"""
