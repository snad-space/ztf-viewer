...
* The Redis half needs ``pytest-redis``, as in ``tests/test_ttl_set_redis_ttl_set.py``.  On macOS
  ...
  tests too).
* Tests marked ``xfail`` document behaviour of today's implementation that looks like a bug (see
  the ``BUG:`` comments).  They are deliberately **non-strict**: if the rewrite fixes them they
  XPASS, which is not a failure, so the rewrite still needs no edit here.  A fixed one should
  simply have its marker dropped in a later, deliberate commit.
"""
...
def expected_bug(request, backends, reason):
    """Mark the running test as a known bug of today's implementation on ``backends``.

    Non-strict on purpose: the rewrite may fix it, and an XPASS keeps this file untouched.
    """
    if request.getfixturevalue("cache_type") in backends:
...
def test_different_kwarg_names_do_not_hit(request, cache):
    ...
    expected_bug(request, {"redis"}, "redis_lru keys on kwargs values only, ignoring names")
    counter = Counter(cache)
...
def test_distinct_functions_do_not_collide(request, cache):
    ...
    expected_bug(request, {"memory"}, "cachetools.cached keys do not include the function")
...
@pytest.mark.xfail(
    reason=(
        "DECISION POINT, not a required fix: today the key depends on the identity of `self`, "
        "so the 16 cached methods share nothing between gunicorn workers and lose everything on "
        "restart even with Redis. Making them key on the class instead would fix that but is "
        "wrong for any instance that carries state. The cache sub-stack decides; either outcome "
        "leaves this file unedited (XPASS is not a failure)."
    ),
    strict=False,
)
...
def test_hashable_value_holding_unhashable_parts_round_trips(request, cache):
    ...
    expected_bug(request, {"redis"}, "redis_lru hashes cached values against its exclude set")
    value = ({"oid": 1}, {"oid": 2})
    counter = Counter(cache, result=value)
    _, second = assert_hit(counter, 1)
    assert second == value
...
def test_unhashable_argument_does_not_raise(request, cache):
    ...
    expected_bug(request, {"memory"}, "cachetools hashes the argument tuple, so a list raises")
    counter = Counter(cache)
