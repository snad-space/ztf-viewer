...
| `ztf_viewer.exceptions.CatalogUnavailable` cannot be constructed without network access to CDS, and the whole test suite fails at *collection* when CDS is unreachable | **Pre-existing, not introduced by any PR in this pass; fixed in flight by #715/#716** — confirmed on `master` while reviewing #711. Two separate import-time network dependencies compounded: `CatalogUnavailable.__init__` imported `ztf_viewer.catalogs.conesearch` just to `isinstance`-check its `catalog` argument, and that package constructs `SimbadQuery`, whose `__init__` calls `add_votable_fields` — which validates field names against SIMBAD's TAP schema over the network. So instantiating the exception required reaching CDS, and importing the catalog package did too. #715 duck-types on `query_name` and drops the conesearch import; #716 builds the SIMBAD client on first query instead of at import, offloaded via `_ensure_coroutine` so the build does not block the loop. With both applied the suite collects and passes in full with CDS unreachable (474 passed, 2 skipped) where it previously aborted during collection. This is why this doc pass, #706, #711 and #712 could not get a full green suite. **Not audited:** the other 18 catalog query objects, which are also constructed at import — only the SIMBAD one was shown to reach the network |
